...
CSV.write(joinpath(base_dir, "tau_PBC.csv"), df_tau)
CSV.write(joinpath(base_dir, "zeta_PBC.csv"), df_zeta)
...
CSV.write(joinpath(base_dir, "g_PBC.csv"), DataFrame(z=z_grid, g_non1=g_pos, g_non2=g_pos_dir))
...
plt_g = plot(z_grid, g_pos; lw=2, color=:red, linestyle=:dot, label="NonLinear1",
    xlabel="Serum bilirubin (mg/dl)", ylabel=L"$g(z)$")
plot!(plt_g, z_grid, g_pos_dir; lw=2, color=:green, linestyle=:dash, label="NonLinear2")
savefig(plt_g, joinpath(base_dir, "g_compare_PBC.pdf"))

# Histogram of Z values used in nonlinear term
plt_z = histogram(Z; bins=30, color=:gray, xlabel="Serum bilirubin (mg/dl)", ylabel="Count")
savefig(plt_z, joinpath(base_dir, "z_hist_PBC.pdf"))
...
CSV.write(joinpath(base_dir, "IBS_PBC.csv"),
